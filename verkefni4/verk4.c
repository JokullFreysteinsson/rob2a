...
task main()
{
	while(true){
		...
		{
			while(SensorValue(sonarSensor) > 60  || SensorValue(sonarSensor) == -1)		// Loop while robot's Ultrasonic sensor is further than 20 inches away from an object
		...
		else if(SensorValue(lightSensor) > 500){
				motor[rightMotor] = 0;
				motor[leftMotor] = 0;
			}
	}
